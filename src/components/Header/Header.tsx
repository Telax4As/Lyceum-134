...
export default function GuestHeader() {
  ...
  return (
    <>
      <header className={`
        sticky top-0 z-[60] w-full border-b backdrop-blur-xl px-4 sm:px-6 py-4 transition-all duration-100 mx-auto flex items-center justify-between
        ${theme === 'light' 
      ...
      `}>
        ...
        <div className="flex items-center gap-3 md:gap-6">
          ...
          <div className="hidden xs:block">
            {role === 'guest' ? <LoginBtn theme={theme} login={t.login} /> : <ProfileBtn theme={theme} profile={t.profile} />}
          </div>
          ...
          >
            <div className={`w-5 h-0.5 mb-1 transition-all duration-100 ${
              theme === 'light' ? 'bg-slate-900' : 'bg-white'
            } ${isOpen ? 'rotate-45 translate-y-1.5' : ''}`} />
            <div className={`w-5 h-0.5 transition-all duration-100 ${
              theme === 'light' ? 'bg-slate-900' : 'bg-white'
            } ${isOpen ? 'opacity-0' : ''}`} />
            <div className={`w-5 h-0.5 mt-1 transition-all duration-100 ${
              theme === 'light' ? 'bg-slate-900' : 'bg-white'
            } ${isOpen ? '-rotate-45 -translate-y-1.5' : ''}`} />
          </button>
      ...
      <div className={`
        fixed inset-0 z-[55] md:hidden transition-all duration-100
        ...
        <div className={`
          absolute right-0 top-0 h-full w-[280px] border-l transition-transform duration-100 p-6 pt-24 flex flex-col
